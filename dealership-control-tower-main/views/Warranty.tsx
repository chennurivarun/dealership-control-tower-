    ...
    <div className="space-y-12 pb-20">
      ...
      <div className="flex items-center gap-2 p-3 glass rounded-[40px] shadow-2xl overflow-x-auto no-scrollbar">
         {[
           ...
           { label: 'Submitted', count: 08, color: 'bg-blue-600' },
           { label: 'OEM Review', count: 05, color: 'bg-indigo-600' },
           { label: 'Needs Info', count: 03, color: 'bg-amber-500' },
           { label: 'Approved', count: 24, color: 'bg-emerald-600' },
         ].map((step, i) => (
           <React.Fragment key={i}>
             <div className="min-w-[160px] flex-1 glass p-6 rounded-[32px] text-center relative overflow-hidden group border border-transparent hover:border-white/10 transition-all">
        ...
        <div className="lg:col-span-8 glass p-10 rounded-[56px] shadow-2xl relative border border-black/5 dark:border-white/5">
          <div className="flex justify-between items-center mb-12">
             ...
             <div className="relative group">
                ...
                <input type="text" placeholder="Search JC No..." className="pl-12 pr-6 py-4 glass rounded-[24px] text-[10px] font-black outline-none uppercase tracking-widest border-none w-56 focus:w-80 transition-all focus:bg-slate-100 dark:focus:bg-charcoal-900 shadow-sm"/>
             </div>
          </div>
             ...
             {MOCK_JOB_CARDS.map((jc) => (
               <div key={jc.id} className="group relative glass p-8 rounded-[40px] hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 flex items-center gap-10 border border-transparent hover:border-blue-600/20">
                  ...
                  <div className="hidden md:block w-32">
                     <span className={`text-[9px] font-black uppercase tracking-[0.2em] px-4 py-2 rounded-2xl block text-center transition-all ${
                       jc.status === 'Approved' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' :
