        ...
        <div className="lg:col-span-8 glass p-12 rounded-[56px] shadow-2xl relative overflow-hidden transition-all border border-black/5 dark:border-white/5">
          <div className="flex justify-between items-center mb-12 relative z-10">
            ...
            <div className="flex bg-slate-100 dark:bg-charcoal-900/50 p-1.5 rounded-2xl border border-black/5 dark:border-white/5 transition-colors">
               <button className="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl">Volume</button>
               <button className="px-6 py-2.5 text-slate-500 hover:text-slate-900 dark:hover:text-slate-300 transition-all rounded-xl text-[10px] font-black uppercase tracking-widest">Revenue</button>
            </div>
          </div>
          <div className="h-96 relative z-10">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data}>
                ...
                />
                <Area type="monotone" dataKey="movements" stroke="#3b82f6" strokeWidth={5} fillOpacity={1} fill="url(#chartGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-8 flex flex-col">
          <div className="flex-1 glass p-10 rounded-[56px] shadow-2xl relative border border-black/5 dark:border-white/5 transition-all bg-white dark:bg-charcoal-900/40">
             ...
             <div className="space-y-8">
                {[
                  { title: 'Overdue Fleet', sub: 'KA03NB (Rahul S) +15m', icon: Car, color: 'text-red-500', bg: 'bg-red-500/10' },
                  { title: 'Spares Hazard', sub: 'Brake Pads @ critical 5%', icon: Zap, color: 'text-amber-500', bg: 'bg-amber-500/10' },
                  { title: 'Identity Confirmed', sub: 'Aman Jha BGV Ready', icon: ShieldCheck, color: 'text-blue-500', bg: 'bg-blue-400/10' }
                ].map((item, i) => (
                  <div key={i} className="group flex gap-6 p-5 rounded-[32px] hover:bg-black/5 dark:hover:bg-white/5 transition-all duration-500 cursor-pointer border border-transparent shadow-sm">
